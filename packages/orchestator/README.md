...
2. **协议层衔接**：利用 `@y-kafka-collabation-server/protocol` 的 codec，把 Socket.IO payload 编码/解码为 Kafka Envelope，并维护本地 `ProtocolCodecContext`。
3. **Kafka 管理**：注入 `topicResolver`、`kafkaProducer`、`kafkaConsumer`，负责 produce/consume 以及 consumer group 生命周期。
...
在与 persistence 集成时，可以注入符合以下接口的对象存储客户端（默认可选）：
